...
public sealed class GetClubActivitiesTool : IStravaTool
{
    private readonly StravaClient _client;
    public GetClubActivitiesTool(StravaClient client) => _client = client;

    public ToolDefinition Definition => new()
    {
        Name = "get_club_activities",
        Description = "List recent activities from members of a specific club.",
    ...
    public async Task<ToolCallResult> ExecuteAsync(JsonObject args)
    {
        ...
        var data = await _client.GetAsync($"/clubs/{id}/activities", qp);
